...
class ModelTrainer():
    def __init__(self, train_params, aug_params, dataset, dataset_list = None):

        # Prints the given trainer parameters
        print("\nList of trainer parameters:")
        self.train_params = train_params
        self.print_dict(self.train_params)

        # Sets the dataset used for training
        self.dataset = dataset
        print("\nTraining models based on '{}' dataset...".format( getattr( self.dataset, "name") ))
        ...
        model_folder   = "mock_models" if self.train_params["mock_train"] else "models"
        self.model_dir = os.path.join( ".", "output", model_folder, getattr( self.dataset, "name") )
    ...
    def gen_model_name( self, hyperparameters ):
    ...
    def train_model( self, hyperparameters, model_path, reset = True ):
        ...
        train_datagen = CustomDataGenerator( self.dataset, "train", hyperparameters, aug_dict = self.aug_params, shuffle = True, 
        ...
        # Checks if a mock training is being performed
        if self.train_params["mock_train"]:
            # If so, sets the number of test steps to low values
            val_steps = np.min( [self.train_params["mock_steps"], val_steps] )
            train_steps = np.min( [self.train_params["mock_steps"], train_steps] )

        # Gets class_weights from training dataset
        class_weights = getattr( self.dataset, "class_weights") if hyperparameters["class_weights"] else None
    ...
    def evaluate_model( self, dataset, hyperparameters, partition ):
        ...
        if self.train_params["mock_train"]:
            # If so, sets the number of test steps to low values
            num_steps = np.min( [self.train_params["mock_steps"], num_steps] )
            num_samples = np.min( [int(hyperparameters["batchsize"] * num_steps), num_samples] )
    ...
    def append_to_csv( self, model_path, model_id, hyperparameters, results ):
        ...
        combined_dict.update( self.aug_params )
    ...
    def hyperparam_to_json( self, model_path, hyperparameters ):
        ...
        dict_of_dicts = { "hyperparameters": hyperparameters, 
                         "augmentation_params": self.aug_params 
                        }
    ...
    def train_test_iteration( self, hyperparameters):
        ...
        model_path, model_id = self.gen_model_name( hyperparameters )

        # Prints current hyperparameters and starts training
        self.print_dict( hyperparameters, round = True )
        history_dict = self.train_model( hyperparameters, model_path )
                
        # Gets the names of the datasets used in training/testing the models
        dataset_name = getattr(self.dataset, "name")
        cval_dataset_names = [ getattr(dset, "name") for dset in self.dataset_list ]
        ...
        self.append_to_csv( model_path, model_id, hyperparameters, results_dict )

        #
        print("\nSaving training hyperparameters as JSON...")
        self.hyperparam_to_json(model_path, hyperparameters)
    ...
    def doGridSearch( self, hyperparam_values ):
        print("\nStarting GridSearch:")
        ...
        hyperparam_permutations = list(self.product_dict(**hyperparam_values))
        print("\nA total of {} hyperparameter permutations were found.".format( len(hyperparam_permutations) ))
        ...
        for idx_h, hyperparameters in enumerate(hyperparam_permutations):
            ...
            print("\n\n#{}/{} Iteration of GridSearch:".format( str(idx_h+1).zfill(3), str(n_permutations).zfill(3) ))
    ...
    def verificar_acc( self, model_path, hyperparameters ):
        ...
        i_col = getattr( self.dataset, "input_col")
        o_col = getattr( self.dataset, "output_col")
        ...
        label_array = tf.keras.utils.to_categorical( np.array( labels ), num_classes = getattr( self.dataset, "n_classes"), dtype = "float32" )
    ...
    def print_dict(dict, round = False):
        ...
        for k, v in dict.items():
            ...
            print("\t{}: {}".format(k.ljust(max_key_length), v))
        return
